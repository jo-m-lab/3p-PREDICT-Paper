---
title: "Iterative tiered clustering (ITC) guided tutorial"
author: 'Kyle Kimler and Ben Doran'
output:
  html_document:
    toc: true
    toc_depth: 2
...
ITC iteratively explores axes of variation in scRNAseq data by clustering and subclustering until variation between cells becomes noise. The philosophy of ITC is that every axis of variation could be biologically meaningful, so each should be explored. Once every possibility is explored, curation and a statistical interrogation of the resolution are used to collapse clusters into the elemental transcriptomes of the dataset. ITC inherently builds a tree of subclustering events. As data is separated by the major axes of variation in each subset, further rounds capture less pronounced variables. This comes with some caveats: variation shared by all celltypes make up one of the major axes of variation in the first round of clustering. Celltypes can split up at the beginning, so the same splitting of e.g. B and T cells might happen further down in separate branches. Thus the resulting clustering tree is neither indicitave of true distances between end clusters nor a tree of unique groupings. We address this by calculation of a binary tree of manually curated end clusters, providing a distance between elemental transcriptomes.  

ITC is meant to be a modular software. Currently it iterates through the default Seurat analysis pipeline, with two additions that allow automation: <br>
...
Ben Doran decided to write the software to address the fact that we were constantly manually subclustering scRNAseq data, and because other iterative clustering softwares like scPanoView and iterclust are tailor built for specific clustering methods outside of the Seurat paradigm.

Model datasets:
<br>
- pediatric FGID: 89849 cells x 22431 genes
<br>
- Nasal Polyp (Ordovas-Montanes et al 2018) scRNAseq: 19196 cells x 22360 genes  

# FGID ITC
Reproducing figures for the 2021 paper  

## Running ITC
The [github page](https://github.com/ShalekLab/SCTieredClustering) describes ITC  
ITC is called directly on a seurat object:  
...
<br>
<img src="https://github.com/jo-m-lab/3p-PREDICT-Paper/raw/master/docs/Tiered-FGID-13_20191028_FGID_tiered_clusters_output_figs_QC_vln_plot.pdf" width="400" height="300">  
<br>
clustering decision metrics  

<img src="https://github.com/jo-m-lab/3p-PREDICT-Paper/raw/master/docs/Tiered-FGID-13_20191028_FGID_tiered_clusters_output_figs_Mon-Oct-28-03_41_07_PM-2019-UTC.pdf" width="800" height="275">  
<br>
...
<img src="https://github.com/jo-m-lab/3p-PREDICT-Paper/raw/master/docs/Tiered-FGID-13_20191028_FGID_tiered_clusters_output_figs_cluster_umap.pdf" width="400" height="400">  
<br>
<br>


  
It also saves subset seurat objects for each cluster in 'srobjs', and saves files with lists of cells per end-cluster in 'endclusts'.  


## Working with ITC results
...
## Add ITC annotations to the seurat object
...
s1 <- ggplot(tierNdiversity,aes(x=diversity))+ geom_histogram() + ggtitle("raw ITC results")
...
itctree <- as.Node(meta) 
print(itctree,'majortype','subtype','groundtruth','curatedname',limit=20)

itcphylo <- as.phylo(itctree)

#For curatednames
#itcphylo$tip.label <- itctree$Get('curatedname')[!is.na(itctree$Get('curatedname'))]
#For tierNident
itcphylo$tip.label <- itctree$Get('tierNident')[!is.na(itctree$Get('tierNident'))]
ggtree(itcphylo,branch.length='edge.length',layout='circular')
...
plotdendrogram <- function(phyloObj,title) {
	...
	ddata_L$count = as.numeric(table(factor(srobj$tierNident))[itcphylo$tip.label])
...
p1 <- plotdendrogram(itcphylo, title='original hierarchy tree')
p1
```
  
  
## Annotation
In the paper, tiers 1 and 2 were annotated manually. End clusters were named based on the tier1.tier2 combination they belonged to, followed by 2 genes either manually chosen or chosen on a heuristic rank score among its tier2 group. The rank score is calculated as follows on the results of a 1 vs. all wilcoxon rank sum test: 
...
We can view the original ITC tree with curated names to get an idea of how we collapsed clusters  
...
While curating the end clusters, we wanted to better understand the distance between clusters. In the atlas figures 3 and 4, we performed hierarchical clustering of curated end clusters (not to be confused with ITC: hclus produces a binary tree). Hierarchical clustering was performed on median expression of a subset of highly DE genes (approximately 4500) using complete linkage and distance calculated with Pearson correlation. DE was performed by pairwise expression tests of end clusters.
...
Here is another example of an ITC tree built on another scRNAseq dataset of nasal polyps.
...
groundtruth <- read_tsv('/Users/kkimler/Projects/SETA/nasalPolyp/20180822_PolypAll_cleaned_metadata.txt') %>% rename(CellID = X1)
...
nasalp1 <- plotdendrogram(itcphylo,'nasal polyp ITC hierarchy')
...
plotdendrogram <- function(phyloObj,title) {
	...
	#ddata_L$group <- itcphylo$tip.group[match(ddata_L$label,itcphylo$tip.label)]
	#For majortype
	ddata_L$group <- gsub("\\..*$", "", ddata_L$label)
	#For subtype
	#ddata_L$group <- gsub("^([^.]+)\\.([^.]+)\\.(.+)$","\\2",ddata_L$label)
...
itcphylo$tip.group <- itctree$Get('papertype')[!is.na(itctree$Get('tierNident'))]

nasalp2 <- plotdendrogram(itcphylo,'nasal polyp curated hierarchy')
...
hclusphylo$tip.group <- itcphylo$tip.group[match(hclusphylo$tip.label,itcphylo$tip.label)]
